...
describe DurableDecorator::Base do
  ...
  # Spec uses ./example_class.rb
    context 'for protected methods' do
      it 'finds them' do
        Namespace::ExampleClass.class_eval do
          durably_decorate :protected_method do
            original_protected_method + " decorated!"
          end
        end

        instance = Namespace::ExampleClass.new
        instance.protected_method.should == 'protected method decorated!'
      end
    end
    ...
    context 'for private methods' do
      it 'finds them' do
        Namespace::ExampleClass.class_eval do
          durably_decorate :private_method do
            original_private_method + " decorated!"
          end
        end

        instance = Namespace::ExampleClass.new
        instance.private_method.should == 'private method decorated!'
      end
    end
    ...
    context 'for existing instance methods' do
      ...
      context 'for re-run decorations' do
        it 'does not create a new redefinition' do
          ...
          DurableDecorator::Base.definitions['Namespace::ExampleClass#no_param_method'].size.should == 2 # original definition and one deoration
        end
      end
      ...
      context 'for double decorations' do
        ...
        it 'maintains history of decorations' do
          DurableDecorator::Base.definitions['Namespace::ExampleClass#one_param_method'].size.should == 2
        end
      end
    ...
    context 'for existing class methods' do
      it 'guarantees access to ::original_method' do
        ...
        Namespace::ExampleClass.clazz_level.should == 'original and a new string'
      end
      ...
      context 'for methods with parameters' do
        it 'guarantees access to ::original_method' do
          ...
          Namespace::ExampleClass.clazz_level_paramed("here we go").should == 'original: check and here we go'
        end
      end
    end
    ...
    context 'when asked to find the sha' do
      ...
      context 'when the target is an instance method' do
        it 'should return the sha' do
          DurableDecorator::Base.determine_sha('Namespace::ExampleClass#no_param_method').should ==
      ...
      context 'when the target is a class method' do
        it 'should return the sha' do
          DurableDecorator::Base.determine_sha('Namespace::ExampleClass.clazz_level').should ==
